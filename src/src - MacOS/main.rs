...
fn get_asset_path(filename: &str) -> String {
    // Pfad zum aktuellen Binary
    let exe_path = std::env::current_exe().unwrap();
    // Verzeichnis des Binaries: .../Contents/MacOS/
    let exe_dir = exe_path.parent().unwrap();

    // Assets liegen bei ../Resources/assets/
    let assets_dir = exe_dir.parent().unwrap().join("Resources").join("assets");
...
async fn main() {
    let jump_sound = load_sound(&get_asset_path("jump.wav")).await.unwrap();
    let game_over_sound = load_sound(&get_asset_path("game_over.wav")).await.unwrap();
    let bg_music = load_sound(&get_asset_path("bg_music.wav")).await.unwrap();

    let dino_texture = load_texture(&get_asset_path("dino.png")).await.unwrap();
    let cactus_texture = load_texture(&get_asset_path("cactus.png")).await.unwrap();
